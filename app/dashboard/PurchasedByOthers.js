...
import { getProgramPurchaseByOthers } from '../store/program/programThunk';
import Image from 'next/image';

const PurchasedByOthers = () => {
	const { uid } = useSelector(state => state?.user);
	const { programPurchaseByOthers } = useSelector(state => state?.program);

	const dispatch = useDispatch();
	useEffect(() => {
		dispatch(getProgramPurchaseByOthers(uid));
	}, []);
	// const programs = [
	//     {
	...
	return (
		<div className='w-full'>
			...
			{programPurchaseByOthers.length > 0 ? (
				<div className='max-w-[1180px] w-full'>
					...
					<div className='md:hidden grid gap-4'>
						{programPurchaseByOthers?.map((program, index) => (
							<div
								key={index}
